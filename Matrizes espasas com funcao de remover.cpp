...
void remover (int numero){
	struct diretor *MelbyLaw = procurarDiretor(numero % MODULO);
	
	struct no *pontAnterior;
	struct no* pontExcluir;
	if (MelbyLaw -> proxNo != NULL){
		pontExcluir = MelbyLaw -> proxNo;
		MelbyLaw -> proxNo = MelbyLaw -> proxNo -> proxNo;
		free(pontExcluir);
		
	}else{
		pontAnterior = MelbyLaw -> proxNo;
		while ((pontAnterior -> proxNo != NULL)&& (pontAnterior -> proxNo -> numero != numero)){
			pontAnterior = pontAnterior -> proxNo;
			free(pontExcluir);
		}
	}
}
